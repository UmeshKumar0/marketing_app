...
class ForeGroundSyncService extends GetxController {
  RxBool syncingData = false.obs;
  RxString syncstatus = "STARTED".obs;
  Future<bool> sync() async {
    ...
    syncstatus.value = 'LOCATION SENT';
    await attendanceSync();
    syncstatus.value = 'ATTENDANCE SYNCED';
    try {
      String status = await getOdometerStatus();
      syncstatus.value = 'ODOMETER STATUS FETCHED';
      if (status == AppConfig.PRESENT) {
        await shopCreateSync();
        syncstatus.value = 'SHOP CREATES SYNCED';
        await syncNormalVisits();
        syncstatus.value = 'NORMAL VISITS SYNCED';
        await syncReminders();
        syncstatus.value = 'REMINDERS SYNCED';
        await syncImages();
        syncstatus.value = 'IMAGES SYNCED';
        await syncMeetings();
        syncstatus.value = 'MEETINGS SYNCED';
